...
if ("HypeReactiveContent" in window === false) window['HypeReactiveContent'] = (function () {
    ...
    function enableReactiveObject(obj, callback, _key) {
        ...
        const cachedProxy = reactiveCache.get(obj);
    ...
    function parseTemplate(template, hypeDocument, elm) {
        // Improved regex to support complex expressions with balanced delimiters
        return template.replace(/{{([\s\S]*?)}}/g, function(match, expression) {
            // Trim whitespace from the expression
            expression = expression.trim();
            
            // Skip escaped expressions like \{{ something }}
            if (match.charAt(0) === '\\') {
    ...
    */
    ...
    function HypeTriggerCustomBehavior(hypeDocument, element, event) {
        ...
        const code = event.customBehaviorName;
